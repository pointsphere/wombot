...
class Room:
    ...
    def getBanlist(self):
        return [record[2] for record in self._banlist]
    ...
    def _addHistory(self, msg):
        ...
        if len(self._history) > self.mgr._maxHistoryLength:
            rest, self._history = (
                self._history[: -self.mgr._maxHistoryLength],
                self._history[-self.mgr._maxHistoryLength :],
            )
            for msg in rest:
                msg.detach()
...
class _User:
    ...
    def getRoomNames(self):
        return [room.name for room in self.getRooms()]
